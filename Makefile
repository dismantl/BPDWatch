...
test: start  ## Run tests
	if [ -z "$(name)" ]; \
	    then FLASK_ENV=testing docker-compose run --rm web pytest -n 16 --dist=loadfile -v tests/; \
	    else FLASK_ENV=testing docker-compose run --rm web pytest -n 16 --dist=loadfile -v tests/ -k $(name); \
	fi
